...
import {
  LayoutDashboard, Truck, Users, Briefcase,
  Wrench, FileText, CreditCard, Phone, Settings, LogOut,
  Menu, X, BookOpen, BarChart2,
} from 'lucide-react';
      ...
      <div className="px-3 py-4 border-t border-slate-700 space-y-3">
        ...
        <div className="flex gap-2">
          <NavLink
            to="/settings"
            onClick={onNav}
            className="flex-1 flex items-center justify-center gap-1.5 bg-slate-600 hover:bg-slate-700 rounded-lg px-2 py-1.5 text-xs text-white transition-colors"
          >
...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-slate-800 text-white flex-shrink-0 flex flex-col transform transition-transform duration-300 ${
      ...
      <main className="flex-1 overflow-auto flex flex-col min-w-0">
        {/* Mobile topbar */}
        <div className="lg:hidden bg-slate-800 text-white px-4 py-3 flex items-center gap-3 flex-shrink-0">
          <button onClick={() => setOpen(true)}>
            <Menu className="w-6 h-6" />
          </button>
          <img src="/logo.png" alt="Go Go Logistica" className="h-8 w-auto object-contain rounded" />
        </div>
